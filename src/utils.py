...
def save_plot(train_loss, val_loss, train_psnr, val_psnr):
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig('/content/drive/outputs/loss.png')
    plt.close()
    # PSNR plots.
    plt.figure(figsize=(10, 7))
    ...
    plt.ylabel('PSNR (dB)')
    plt.legend()
    plt.savefig('/content/drive/outputs/psnr.png')
    plt.close()

def save_model_state(model):
    # save the model to disk
    print('Saving model...')
    torch.save(model.state_dict(), '/content/drive/outputs/model.pth')
def save_model(epochs, model, optimizer, criterion):
    ...
    torch.save({
                ...
                'loss': criterion,
                }, f"/content/drive/outputs/model_ckpt.pth")
...
def save_validation_results(outputs, epoch, batch_iter):
    ...
    save_image(
        outputs, 
        f"/content/drive/outputs/valid_results/val_sr_{epoch}_{batch_iter}.png"
    )
